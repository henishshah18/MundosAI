...
from app.models.patient import PatientType, ChannelType
from app.models.appointment import AppointmentStatus, CreatedFrom
...
async def dashboard_stats() -> Dict[str, Any]:
    ...
    booked_month = 0
    for appt in await db["appointment"].find_many({}):  # type: ignore[attr-defined]
    ...
    handoffs = await db["campaigns"].count_many({"status": CampaignStatus.HANDOFF_REQUIRED.value})  # type: ignore[attr-defined]
    active_recovery = await db["campaigns"].count_many({
    ...
    total_recovery = await db["campaigns"].count_many({"campaign_type": CampaignType.RECOVERY.value})  # type: ignore[attr-defined]
    recovered = await db["campaigns"].count_many({"status": CampaignStatus.RECOVERED.value})  # type: ignore[attr-defined]
    recovery_rate = (recovered / total_recovery * 100.0) if total_recovery else 0.0

    total_recall = await db["campaigns"].count_many({"campaign_type": CampaignType.RECALL.value})  # type: ignore[attr-defined]
    recall_recovered = await db["campaigns"].count_many({
...
async def list_campaigns(
    ...
    items: List[Dict[str, Any]] = await db["campaigns"].find_many(query, sort=[("updated_at", -1)])  # type: ignore[attr-defined]
    total = len(items)
    start = (page - 1) * limit
    end = start + limit
    ...
    for c in page_items:
        patient = await db["patients"].find_one({"_id": c.get("patient_id")})
        results.append(
            {
...
async def campaign_details(campaign_id: str) -> Dict[str, Any]:
    ...
    messages = await db["interactions"].find_many({"campaign_id": oid})  # type: ignore[attr-defined]
    history = [
        {
    ...
    patient = await db["patients"].find_one({"_id": campaign.get("patient_id")})
    details = {
...
async def list_appointments(
    ...
    for appt in await db["appointment"].find_many({}):  # type: ignore[attr-defined]
        ...
        patient = await db["patients"].find_one({"_id": appt.get("patient_id")})
        results.append(
            {
...
async def create_recovery_campaign(payload: RecoveryCampaignCreate) -> Dict[str, str]:
    ...
    presult = await db["patients"].insert_one(patient_doc)

    # Create campaign
    campaign_doc = {
        "patient_id": presult.inserted_id,
        "campaign_type": CampaignType.RECOVERY.value,
        "status": CampaignStatus.ATTEMPTING_RECOVERY.value,
        "engagement_summary": payload.initial_inquiry,
    }
    cresult = await db["campaigns"].insert_one(campaign_doc)
    return {"message": "Recovery campaign created successfully.", "campaign_id": str(cresult.inserted_id)}
...
async def respond_to_campaign(campaign_id: str, payload: CampaignRespondRequest) -> Dict[str, str]:
    ...
    await db["campaigns"].update_one({"_id": oid}, {"$set": {"status": payload.new_status}})
...
async def create_admin_appointment(payload: AdminAppointmentCreate) -> Dict[str, Any]:
    db = await get_database()
    patient = await db["patients"].find_one({"email": str(payload.email)})
    if not patient:
        # create patient
        patient_doc = {
            "name": payload.name,
    ...
    aresult = await db["appointment"].insert_one(appt_doc)
    appt_doc["_id"] = aresult.inserted_id
    return appt_doc
...
async def complete_appointment(appointment_id: str, payload: CompleteAppointmentRequest) -> Dict[str, str]:
    ...
    appointment = None
    if oid is not None:
        appointment = await db["appointment"].find_one({"_id": oid})
    if appointment is None:
        appointment = await db["appointment"].find_one({"_id": appointment_id})
    if not appointment:
        raise HTTPException(status_code=404, detail="Appointment not found")

    # Step 1: update appointment status
    if oid is not None:
        await db["appointment"].update_one({"_id": oid}, {"$set": {"status": AppointmentStatus.completed.value}})
    else:
        await db["appointment"].update_one({"_id": appointment_id}, {"$set": {"status": AppointmentStatus.completed.value}})

    # Step 2: update campaign status to RECOVERED if campaign_id exists
    campaign_id = appointment.get("campaign_id")
    if campaign_id:
        await db["campaigns"].update_one({"_id": campaign_id}, {"$set": {"status": CampaignStatus.RECOVERED.value}})

    # Step 3: handle future recall
    if payload.next_follow_up_date is not None:
        patient_id = appointment.get("patient_id")
        await db["patients"].update_one(
            {"_id": patient_id}, {"$set": {"next_follow_up_date": payload.next_follow_up_date}}
        )
...
async def delete_appointment(appointment_id: str) -> Dict[str, str]:
    ...
    await db["appointment"].delete_one(query)
